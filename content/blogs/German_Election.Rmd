...
The Guardian newspaper has an [election poll tracker for the upcoming German election](https://www.theguardian.com/world/2021/aug/20/german-election-poll-tracker-who-will-be-the-next-chancellor). The list of the opinion polls since Jan 2021 can be found at [Wikipedia](https://en.wikipedia.org/wiki/Opinion_polling_for_the_2021_German_federal_election). Here we will reproduce the graph with the following code. As a referejce, please see the picture from [The Guardian](https://www.theguardian.com/world/2021/aug/20/german-election-poll-tracker-who-will-be-the-next-chancellor) below that is replicated with the code. However, we choose to only plot the values for 2021. Essentially, we look at all voting polls from 2021 and calculate a 14-day moving average which we then plot. We do this to see the changes in voting forecasts leading up to the german federal election in September 2021. 
...
Then we create a dataframe with the columns of data we are actually interested in for the analysis. In addition we mutate the dataframe so that all infromation we need to create the graph is created. In specific, we calculate the 14-day moving average poll prediction for the individual parties. 
...
Finally, we actually plot the graph using the mutated data frame from before. We ensure that the lines have the correct color depending on the party, adjust the axis and label the graph.
